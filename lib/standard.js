...
class StandardEmailAddress {
  ...
  parseRFC() {
    var tokenMatch, strMatch;
    if (this.config.localEncoding === 'unicode') {
      tokenMatch = /^([\p{L}\p{N}\-\!\#\$\%\&\'\*\+\-\/\=\?\^\_\`\{\|\}\~\(\)]+)(.*)/i;
      strMatch   = /^(\"(?:\\[\"\\]|[\x20-\x21\x23-\x2F\x3A-\x40\x5B\x5D-\x60\x7B-\x7E\p{L}\p{N}])+\")(.*)/i;
    } else {
      tokenMatch = /^([\w\!\#\$\%\&\'\*\+\-\/\=\?\^\_\`\{\|\}\~\(\)]+)(.*)/i;
      strMatch   = /^(\"(?:\\[\"\\]|[\x20-\x21\x23-\x2F\x3A-\x40\x5B\x5D-\x60\x7B-\x7EA-Za-z0-9])+\")(.*)/i;
                   //                  sp  !   #    /   :    @   [   ]    `   {    ~  AZaz  09
    }
