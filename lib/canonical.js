...
class CanonicalEmailAddress extends StandardEmailAddress {
  ...
  localString() {
    let loc = this.local.account;
    if (this.config.mailboxCanonical) {
      loc =this.config.mailboxCanonical(loc);
    }
    return loc;
  }
...
module.exports = CanonicalEmailAddress;
